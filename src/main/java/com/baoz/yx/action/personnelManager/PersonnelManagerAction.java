...
public class PersonnelManagerAction extends DispatchAction implements ServletRequestAware  {
	...
	private void prepareUpdateData(Long id){
		dutyList=commonService.list("from OrganizationTree gc where gc.is_active = 1");
		info=queryService.listQueryResult(" select e,g from Employee e,OrganizationTree g where e.is_active=1 and g.is_active=1 and e.position = g.id and e.id='"+id+"'", info);
	...
	{	
		...
		logger.info(password);
		logger.info(empId);
		commonService.executeUpdate("update Employee e set e.password = '" + MD5.toMD5(password) + "' where e.id = " + empId);
		logger.info("修改密码成功!");	
	...
	public String delClient() throws Exception
	{
		logger.info("删除关联");
		logger.info(cliId);
		commonService.executeUpdate("delete from YXOEmployeeClient t where t.id="+cliId);
		return refreshPage();
	}
