...
function openUrpl(param){
	...
	var url = "/yx/contract/searchContractOkJCList.action?method=changeTrueAndJC&conMainId="+param;
	var form = document.getElementById("searchContractOkJCList");
	form.target = "xx";
	form.action = url;
	form.submit(); 
	form.action="/yx/contract/searchContractOkJCList.action"; 
	form.target = "_self"; 
	//window.open('/yx/contract/searchContractOkJCList.action?method=changeTrueAndJC&conMainId='+param,'','menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=1,resizable=1,height=170,width=300');
}
function selAll(){   
...
if(flag==false){
     ...
     if(count==1){
              ...
              if(flag==1){
                  ...
                  document.forms(0).action="/yx/contract/searchContractOkJCList.action?method=changeTrueState";
                  document.forms(0).submit();
              }
              if(flag==2){
                 if(imp){
                    document.forms(0).action="/yx/contract/searchContractOkJCList.action?method=changeTrueAndJC";
                    document.forms(0).submit();
...
if(convar.value=='确认退回'){
       ...
       document.forms(0).action="/yx/contract/searchContractOkJCList.action?method=confirmReturn";
       document.forms(0).submit();
       }
      ...
      document.forms(0).action="/yx/contract/searchContractOkJCList.action?method=changeOrderState";
      document.forms(0).submit();
      }
}
else{
     ...
     document.forms(0).action="/yx/contract/searchContractOkJCList.action?method=cancelConfirm";
     document.forms(0).submit();  
	...
	function confirmCancel(){
	    ...
	    if(flag==1){
			...
			if(chegth==true){
			      openWin("searchContractOkJCList.action?method=showConfirmCancelJC&"+$(searchContractOkJCList).toQueryString(),400,300);
			}
		}else{
			alert("请选择一个合同！");
		}
...
<s:form action="searchContractOkJCList" target="content" theme="simple">
...
<s:hidden name="XMJC" value="2"></s:hidden>
<input type="hidden" name="states"/>
<div align="left">
  <p  style="color:#000000">当前页面：合同管理 -> 合同确认(集成类)</p>
</div>
...
<table width="100%" border="0" cellspacing="1" cellpadding="1" class="bg_white" align="center" >
  ...
  <tr>
    ...
    <table width="100%" border="1" align="center" id="sureCon" bordercolor="#808080" style=" border-collapse: collapse;">
        <tr>
