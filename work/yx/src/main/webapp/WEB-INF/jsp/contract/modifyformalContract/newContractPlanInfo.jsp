...
<table width="100%" height="100%" border="0"  align="center" cellpadding="1" cellspacing="1" class="bg_table02">
  ...
  <tr class="bg_table02">
    ...
    <div id="content4"  >  
                ...
                <s:iterator value="planlist"  id="plan"  status="ilist">
                 <s:if test="moneytype == billNature">
	                <tr>
	                   ...
	                   <td align="center" width="25%" ><s:property value="finalToCloseService.findChangeStageName(conItemStage)" /></td>
	                   <td align="center" width="25%" >
	                   <s:if test="contractItemMaininfo==null">
	                    <s:property value="finalToCloseService.findChangeDepNameByMainid(mainid)"/>
	                   </s:if>
	                   <s:else>
	                    <s:property value="finalToCloseService.findChangeDepName(contractItemMaininfo)" />
	                   </s:else>                  
	                   </td>
        ...
        <div align="center">
          <input  type="button" value="保存"  onClick="if(!validate()){setNum(4);document.formalContractModify.submit();}" class="button01" >   
...
function setValueToItemAndStage(pid){
	var mid = document.formalContractModify.mainid.value;
	var selectNode=document.formalContractModify.elements("changeRealPlan.conItemStage");
	//给阶段list赋值
	ajaxSetSelectOptions("<s:url includeParams="none" action="jsonData"></s:url>?method=doGetStageByConAndPart&mainid="+mid+"&partId="+pid,document.formalContractModify.elements("changeRealPlan.conItemStage"),"key","value",{defaultAsync:false,value:"",text:"--请选择--"});
	//给项目list赋值
	ajaxSetSelectOptions("<s:url includeParams="none" action="jsonData"></s:url>?method=doGetItemByConAndPart&mainid="+mid+"&partId="+pid,document.formalContractModify.elements("changeRealPlan.contractItemMaininfo"),"key","value",{defaultAsync:false,value:"",text:"--请选择--"});
}
function addRealPlan()
{
	if(!validateAdd()){
		document.formalContractModify.method.value="addChangeRealPlan";
		document.formalContractModify.submit();
	}
}
function validateAdd(){
	var ev2 =new Validator();
	var pselectNode = document.formalContractModify.elements("partId").selectedIndex;
	var itemSelectNode = document.formalContractModify.elements("changeRealPlan.contractItemMaininfo").selectedIndex;
	var stageSelectNode = document.formalContractModify.elements("changeRealPlan.conItemStage").selectedIndex;
	if(pselectNode==0){
...
function validate(){
	var ev2=new Validator();
	var count = document.formalContractModify.planIndex.value;
	for(var k=0;k<count;k++){
		var billMoney = document.formalContractModify.elements("planlist["+k+"].realBillAmount").value;
		var receMoney = document.formalContractModify.elements("planlist["+k+"].realReceAmount").value;
...
function tijiao()
{
	document.formalContractModify.method.value="applySubmitByPlan";
	document.formalContractModify.submit();
}
...
function operationbalanceAllPart(){
  ...
  var i=document.getElementById("planIndex").value;
  for(var j=0;j<i;j++){
    ev2.test("float","开票金额必须是数字!",document.getElementById("formalContractModify_planlist_"+j+"__realBillAmount").value);
  	ev2.writeErrors(errorsFrame, "errorsFrame");
  	if(ev2.size()>0){
  	  return;
  	}
...
function operationbalanceByPart(moneyType){
  ...
  var i=document.getElementById("planIndex").value; 
   var alltextmoney = 0;
   for(var k=0;k<i;k++){
     var money = document.getElementById("formalContractModify_planlist_"+k+"__realBillAmount");
