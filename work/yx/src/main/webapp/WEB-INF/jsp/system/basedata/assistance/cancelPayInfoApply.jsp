...
<script src="/yx/commons/scripts/mootools-1.2-core-jm.js"
	...
	<script type="text/javascript">
		...
		function cancelApplyPay(){
			var flag = window.confirm("您确定要取消这些付款申请吗？");
			if(flag){
				var checkFlag = checkboxChecked();
				if(checkFlag){
					document.systemOpAssistance.method.value = "cancelPayApply";
					document.systemOpAssistance.submit();
				}else{
					alert("请选择您要操作的付款申请单！");
				}	
...
<body>
<s:iterator id="erMessage" value="result.errorMessages">
	<font color="red"><strong><s:property value="#erMessage"/></strong></font><br/>
</s:iterator>
<s:form action="systemOpAssistance" theme="simple">
	<s:hidden name="method" value="searchAssiConThenCancelPayApply"></s:hidden>
		...
		<td>
			<table>
				<tr>
					...
					<s:if test="cancelSign != null && cancelSign == 1">
					<td>
					<input type="button" value="取消付款申请" class="button01" onclick="cancelApplyPay();"/>
					</td>
					</s:if>
				</tr>
			</table>
