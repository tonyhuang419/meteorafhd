...
//验证通过表单提交
	function doSave(){
		if(!validate()){
				document.forms(0).action = "<s:url includeParams="none" action="assistanceTicket"><s:param name="method">saveAssistanceTicket</s:param></s:url>";
	...
	function validate(){
		...
		with(assistanceTicket){
		       ...
		       ev2.test("+float+0","发票金额不是数字,请输入0~9数字!",$('at.money').value);
		        ev2.test("notblank","开具日期为空,请输入开具日期!",$('ticketDate').value);
		       ev2.test("dateYX","开具日期时间格式不正确!",$('ticketDate').value);
		   }  
		  if (ev2.size() > 0) {
	...
	function checkPayNo(obj)
	{
		...
		var method = "checkPayNoNotExists";
		 var myRequest = new Request({url:url,async:false,method:'get'});
		   ...
		   myRequest.send("method="+method+"&ticketNo="+payNo+"&randomNum="+Math.random());
	}
	function checkMoney(){
		...
		var selectNode = document.assistanceTicket.elements("at.type");
		var ticketName = selectNode.options[selectNode.selectedIndex].text;
		  var myRequest = new Request({url:url,async:false,method:'get'});
		   myRequest.addEvent("onSuccess",function(responseText, responseXML){
		      var result = JSON.decode(responseText);
			  if(result.isOverAmount){
			  	 obj.value="";
			     ev2.addError("你输入的"+ticketName+"金额不能超过:"+result.amount);
			     ev2.writeErrors(errorsFrame, "errorsFrame");  
			  } 
		    });
		myRequest.send("method="+method+"&assistanceConId="+assistanceConId+"&ticketMoney="+money+"&ticketType="+ticketType+"&randomNum="+Math.random());
...
<s:form action="assistanceTicket" theme="simple">
	...
	<s:hidden name="fromContractManager"></s:hidden>
      ...
      <tr>
        ...
        <table width="100%" border="0" cellspacing="1" cellpadding="1">
          <tr align="center">
           <td width="13%" align="right" class="bg_table02">外协合同名称：</td>
            <td width="37%" align="left" class="bg_table02">
            	...
            	<s:if test="fromContractManager==1">
            	</s:if>
            	<s:else>
          ...
          <tr align="center">
            ...
            <td class="bg_table02" align="left"><s:textfield name="at.num" size="15" maxlength="15" onkeyup="this.value=this.value.replace(/\D/g,'')"  onblur="checkPayNo(this);"></s:textfield></td>
          ...
          <tr align="center">
            ...
            <td align="left" class="bg_table02">
              <s:textfield name="at.money" onblur="checkMoney();" size="15"></s:textfield></td>
          ...
          <tr class="bg_table03" align="center" style="height:42px">
            <td colspan="4"><table style="width:0%;100%">
                <tfoot class="bg_table03" style="height:42px">
                  <tr>
                    <td align="right" colspan="5"><input class="button01" type="button" name="gonext" value="保    存" onClick="javascript:doSave();">                    
                  	<input type="button" value="关闭" class="button01" onclick="window.close();"/> </td>
                  </tr> 
                </tfoot>
            </table></td>
          </tr>
