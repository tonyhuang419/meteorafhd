...
//收款计划变更统计
@Results( {
	@Result(name = "SUCCESS", value = "/WEB-INF/jsp/statistics/mainRecePlanChange.jsp"),
	@Result(name = "leftQuery", value = "/WEB-INF/jsp/statistics/recePlanChangeLeft.jsp"),
	@Result(name = "mainInfo", value = "/WEB-INF/jsp/statistics/recePlanChangeMainInfo.jsp"),
	@Result(name = "showDetails", value = "/WEB-INF/jsp/statistics/popShowRecePlanDetails.jsp")
})
public class ReceProjectChangeStatAction extends DispatchAction {
	...
	private ObjectPropertySessionHoldTool holdTool = new ObjectPropertySessionHoldTool(
			"receProjectChangeStatParameters", this, new String[] { "groupId",
					"saleMan", "changeCountStart", "changeCountEnd",
					"changeType", "changeDateStart", "changeDateEnd",
					"customerName", "customer", "contractNo", "itemNo" });

	private BigDecimal totalPreRece;//预计收款金额总计
	...
	public String showMainInfo() throws Exception {
		...
		sql.append("select emp.name as eName," +
				...
				"(select max(his.change_time) from yx_con_bcplan_change_his his where his.fk_real_con_billpro_sid=p.real_con_billpro_sid and his.history_type='1')," +
				...
				"(select his.before_rece_date from yx_con_bcplan_change_his his where his.fk_real_con_billpro_sid=p.real_con_billpro_sid and his.history_type='1' and his.billpro_change_his_sid=(select max(hs.billpro_change_his_sid) from yx_con_bcplan_change_his hs where hs.fk_real_con_billpro_sid=p.real_con_billpro_sid and hs.history_type='1' )) as actualReceDate ," +
		...
		"and tm.type_big=1023 and p.rece_change_count>0 ");
		if(groupId!=null){
		...
		if(changeDateStart!=null&&changeDateStart.trim().length()>0){
			conditionsHql.append(" and to_date('"+changeDateStart+"','yyyy-MM-dd') <= trunc((select max(his.change_time) from yx_con_bcplan_change_his his where his.fk_real_con_billpro_sid = p.real_con_billpro_sid and his.history_type='1'),'dd')");
		}
		if(changeDateEnd!=null&&changeDateEnd.trim().length()>0){
			conditionsHql.append(" and to_date('"+changeDateEnd+"','yyyy-MM-dd') >= trunc((select min(his.change_time) from yx_con_bcplan_change_his his where his.fk_real_con_billpro_sid = p.real_con_billpro_sid and his.history_type='1'),'dd')");
		}
		if(changeCountStart!=null){
			conditionsHql.append(" and p.rece_change_count>="+changeCountStart+" ");
		}
		if(changeCountEnd!=null){
			conditionsHql.append(" and p.rece_change_count<="+changeCountEnd+" ");
		}
		if(changeType!=null&&changeType.trim().length()>0){//bill_change_type
			conditionsHql.append(" and (select count(*) from yx_con_bcplan_change_his his where his.fk_real_con_billpro_sid=p.real_con_billpro_sid and his.rece_change_type='"+changeType+"' and his.history_type='1')>0");
		}			
		if(customer!=null){
	...
	public String popShowDetails()throws Exception{
		...
		String hql="from BillReceChangeHistory his where his.realContractBillandRecePlan=? and his.historyType='1' order by his.billproChangeHisSid";
	...
	private  void processExport(List<Object[]> oArray) {	
		...
		export.addTitle(new String[]{"销售员","合同号","项目号","合同名称","客户名称","新收款日期","原始收款日期"
				,"预计收款金额","变更数","合同阶段","最后变更日期" });
		export.setTableName("收款计划变更统计");
		for(Object[] obj:oArray){
			stageInfo = obj[9].toString();
			if(obj[10]!=null){
				stageInfo += stageInfo +"("+ obj[10].toString()  +")"; 
			}
			export.addRow(new Object[]{obj[0],obj[1],obj[2],obj[3],obj[4],obj[5],obj[12],obj[7],obj[8].toString(),stageInfo,obj[6] });
		}
		OutputStream os = DownloadUtils.getResponseOutput("收款计划变更统计.xls");
		export.export(os);
