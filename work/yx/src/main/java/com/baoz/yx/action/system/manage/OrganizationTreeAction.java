...
@Results( {
		@Result(name = "success", type = ServletRedirectResult.class, value = "/system/manage/organizationTreeQuery.action?hasChild=${#hasChild}&hasEmployee=${#hasEmployee}"),
...
public class OrganizationTreeAction extends DispatchAction {
	...
	public String saveChildNode() {
		...
		List tempList = commonService.list("from OrganizationTree ot where ot.is_active='1' and ot.organizationCode like ? order by ot.organizationCode", organizationCode + "__");
	...
	public String deleteNode() {
		...
		OrganizationTree orgTree=(OrganizationTree) commonService.load(OrganizationTree.class,organizationId);
		//是不是有自节点
		Number child = (Number) commonService.uniqueResult("select count(*) from OrganizationTree t where t.is_active = 1 and id <> "+organizationId+" and t.organizationCode like '"+orgTree.getOrganizationCode()+"%'");
		if(child.intValue() >0){
			//有子
			ActionContext.getContext().put("hasChild", true);
			return SUCCESS;
		}
		//是不是有员工引用
		Number emp = (Number) commonService.uniqueResult("select count(*) from Employee t where t.is_active = 1 and t.position = "+organizationId);
		if(emp.intValue() >0){
			//有员工引用
			ActionContext.getContext().put("hasEmployee", true);
			return SUCCESS;
		}	
		if(child.intValue()<1&&emp.intValue()<1)
		{
