		 ...
		 if (contractProList != null && contractProList.size() > 0){
			for (ContractItemMaininfo Self:contractProList) 
			{
				if (Self.getConItemMinfoSid() != null)
				{
				    ...
				    contractservice.itemIsCloseByItem(toDel.getConItemMinfoSid());
				}
			}
		 logger.info("确认通过->成功");
		 }
		...
		logger.info("取消确认");	
		 if (contractProList != null && contractProList.size() > 0){
			for (ContractItemMaininfo Self:contractProList) 
			{
				...
				{
					ContractItemMaininfo toDel = (ContractItemMaininfo) service.load(ContractItemMaininfo.class, Self.getConItemMinfoSid());
					ContractMainInfo cmaininfo = (ContractMainInfo)service.load(ContractMainInfo.class,toDel.getContractMainInfo());
					toDel.setConItemCostSure(2L);
				    toDel.setConItemState(0L);
				    cmaininfo.setConState(4L);
				    service.update(cmaininfo);
				    service.saveOrUpdate(toDel);
				}
			}
		 logger.info("取消确认->成功");
