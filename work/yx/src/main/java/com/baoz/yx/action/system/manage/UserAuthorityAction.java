...
@Results( {
		...
		@Result(name = "updateSuccess", value = "/commons/jsp/closeAndCallBackOpener.jsp"), 
...
public class UserAuthorityAction extends DispatchAction {
	...
	public String editAuthority() throws Exception {
		...
		authorityList = commonService.
			list("select a, (select count(*) from AuthorityEmployee ae where ae.authorityId=a.id and ae.userId=? ) , (select count(*) from RoleAuthority r ,RoleEmployee re where re.roleId = r.roleId and r.authorityId = a.id and re.userId=? ) from Authority a order by a.code", userId , userId);
