...
@Results( { @Result(name = "success", value = "/WEB-INF/jsp/contract/contractItemUpdateRight.jsp"),@Result(name="goupdate",value="/WEB-INF/jsp/contract/updateConItemId.jsp"),@Result(name="flush",value="/WEB-INF/jsp/contract/flushUpdateItem.jsp") })
public class ContractItemUpdateManagerAction extends DispatchAction {
	@Autowired
	...
	public String doDefault() throws Exception {
		...
		sp
				.append("select c,e,y,cm from ContractMainInfo c,Employee e,YXClientCode y,ContractItemMaininfo cm,OrganizationTree orgTree where c.saleMan=e.id and c.conCustomer=y.id and c.conMainInfoSid=cm.contractMainInfo and e.position=orgTree.id and c.conId is not null");
		...
		if (groupId != null && !"".equals(groupId)) {
			sp.append(" and orgTree.organizationCode like '").append(
					groupId + "%'");
		}
		...
		sp.append(" order by cm.conItemMinfoSid desc ");
	...
	public String saveItemNo() throws Exception {
		 ...
		 * logger.info("itemsIdLength================"+itemsId.length); for(int
		 * i=0;i<itemsNo.length;i++){
		 * 
	...
	public String updateConItemId() throws Exception {
		ContractItemMaininfo citem = contractService
				.getContractItemMaininfo(resNo);
	...
	public String doUpdateConItemId() throws Exception {
		...
		logger.info("doUpdateConItemId   upResNubmer====================="
				+ upResNubmer);
		ContractItemMaininfo citem = contractService
				.getContractItemMaininfo(resNo);
		ContractItemMaininfo citemRedo=contractService.findContractItemMaininfoByItemNo(upResNubmer);
		logger.info("doUpdateConItemId   upResNubmer====================="
				+ citemRedo);
		if(citemRedo!=null){
			   setMessage("项目号已存在,请重新输入!");
			   return "goupdate";
			}else{
		String oldItemNo=citem.getConItemId();
		citem.setConItemId(upResNubmer);
		String newItemNo=citem.getConItemId();
	...
	public String getUpResNubmer() {
		return upResNubmer;
	}

	public void setUpResNubmer(String upResNubmer) {
		this.upResNubmer = upResNubmer;
	}
