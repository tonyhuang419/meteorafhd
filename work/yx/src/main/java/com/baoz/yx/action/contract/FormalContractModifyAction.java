...
@Results( {
		...
		@Result(name = "applySubmitSuccess", value = "/WEB-INF/jsp/contract/modifyformalContract/applySubmitSuccess.jsp")
})
public class FormalContractModifyAction extends DispatchAction {
	...
	@Autowired
	@Qualifier("alterationContractService")
	private IAlterationContractService alterationContractService;
	...
	private List<ChangingContractItemStagePlan> addStagePlanList;//添加的stagePlanList
	...
	private int stageIndex;//阶段第几个
	
	private ChangingContractMaininfoPart changMainInfoPart;// 合同费用组成对象
	


	public ChangingContractMaininfoPart getChangMainInfoPart() {
		return changMainInfoPart;
	}



	public void setChangMainInfoPart(ChangingContractMaininfoPart changMainInfoPart) {
		this.changMainInfoPart = changMainInfoPart;
	}
	...
	public String doDefault() {
	    ...
	    if(ca!=null){
	    	...
	    	maininfo = alterationContractService.loadandcopyContractMainInfotoclose(mainid);
		}
	...
	public String saveMainInfo() throws Exception {
		...
		maininfo = alterationContractService.saveContractInfo(maininfo);
		logger.info("保存完毕，跳转至其他页面");
		mainid = maininfo.getConMainInfoSid();		
		logger.info(changeExplain);
        if(mainMoneyList!=null){
        	alterationContractService.saveContractMaininfoPart(mainMoneyList,mainid);		
        }    
 		return toPage(mainid);

	}
	
	public String addMainInfoPart()throws Exception{
		
		/******
		 * 保存合同费用组成信息
		 */
		changMainInfoPart = new ChangingContractMaininfoPart();
		changMainInfoPart.setContractmainid(mainid);
		changMainInfoPart.setMoney(mainmoney);
		changMainInfoPart.setMoneytype(mainmoneytype);
		changMainInfoPart.setTicketType(ticketType);
		alterationContractService.addMainInfoPart(changMainInfoPart);
		///////////
		
		ChangingContractMainInfo cminfo = (ChangingContractMainInfo) commonService.load(ChangingContractMainInfo.class, mainid);
		//////修改合同含税和不含税金额
		if(cminfo.getStandard().equals("1")){
			cminfo.setConTaxTamount(maininfoMoney);
			cminfo.setConNoTaxTamount(alterationContractService.accountnoTaxMoney(mainid));  
		}
		if(cminfo.getStandard().equals("2")){
			cminfo.setConNoTaxTamount(maininfoMoney);
			cminfo.setConTaxTamount(alterationContractService.accountTaxMoney(mainid));
		}
		commonService.update(cminfo);
		return toPage(mainid);
	}
	...
	public String applySubmit()throws Exception{
		...
		if(result==0){
			alterationContractService.applySubmit(mainid,"1");
	...
	public String applySubmitByPlan()throws Exception{
		...
		if (planlist != null) {
			...
			alterationContractService.savePlanInfo(planlist,mainid);
		}
		tag =4;
		return  applySubmit();
	}
	public String applySubmitByStagePlan()throws Exception{
		...
		alterationContractService.saveStagePlan(stagePlanlist);
		tag = 2;
		return applySubmit();
	}
	public String applySubmitByItemInfo()throws Exception{
		logger.info("合同项目信息页面提交");
		alterationContractService.saveItemMainInfo(itemInfoList);
		tag = 3;
		return applySubmit();
	}
	...
	public String addEventInfo()throws Exception{
		if (money.get(saveIndex) != null && partInfoList.get(saveIndex) != null
			...
			// 保存项目信息，合同id
			ChangingContractItemMaininfo citemMainInfo = new ChangingContractItemMaininfo();
			citemMainInfo.setContractMainInfo(mainid);
			// 项目负责部门
			citemMainInfo.setItemResDept(itemResDept.get(saveIndex));
			// 部门负责人
			citemMainInfo.setItemResDeptP(itemResDeptP.get(saveIndex));
			citemMainInfo.setConItemCostSure(0L);
			// 保存项目费用信息，项目id，合同费用id
			ChangingContractItemInfo citemInfo = new ChangingContractItemInfo();
			citemInfo.setMainInfoPartId(partInfoList.get(saveIndex));
			citemInfo.setConItemAmountWithTax(BigDecimalUtils.toBigDecial
			...
			alterationContractService.AddEventInfo(citemMainInfo, citemInfo);
		}
		return toPage(mainid);
	...
	public String addStageInfo() throws Exception{
		if(addStagePlanList!=null&&addStagePlanList.size()>0){
			ChangingContractItemStagePlan citemStagePlan = addStagePlanList.get(stageIndex);
			citemStagePlan.setContractMainSid(mainid);
			alterationContractService.addStageInfo(citemStagePlan);
		}
		return toPage(mainid);
	...
	public String addChangeRealPlan()throws Exception{
		alterationContractService.addChangeRealPlan(mainid, changeRealPlan,partId);		
		return toPage(mainid);
	}
	...
	public IAlterationContractService getAlterationContractService() {
		return alterationContractService;
	}



	public void setAlterationContractService(
			IAlterationContractService alterationContractService) {
		this.alterationContractService = alterationContractService;
	}



	public List<ChangingContractItemStagePlan> getAddStagePlanList() {
		return addStagePlanList;
	}



	public void setAddStagePlanList(
			List<ChangingContractItemStagePlan> addStagePlanList) {
		this.addStagePlanList = addStagePlanList;
	}
