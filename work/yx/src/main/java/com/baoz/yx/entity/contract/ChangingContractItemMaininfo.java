...
//合同项目主体信息表
@Entity
@Table(name = "yx_changing_con_item_minfo")
@Cache(usage = CacheConcurrencyStrategy.NONSTRICT_READ_WRITE)
@org.hibernate.annotations.Entity(selectBeforeUpdate = true, dynamicInsert = true, dynamicUpdate = true)
@Proxy(lazy = false)
public class ChangingContractItemMaininfo implements Serializable {
	...
	private Long conItemCostSure; // 合同项目成本确认状态 //0未录入 1已录入 2待确认 3确认通过
									// 4确认退回,5签收确认，6保留
	...
	@OneToMany(targetEntity = ChangingContractItemInfo.class, mappedBy = "contractItemMaininfo")
	@JoinColumn(name = "fk_con_item_minfo_sid")
	@OrderBy(value = "conItemInfoSid")
	private List<ChangingContractItemInfo> changeContractItemInfolist;
	...
	public List<ChangingContractItemInfo> getChangeContractItemInfolist() {
		return changeContractItemInfolist;
	}

	public void setChangeContractItemInfolist(List<ChangingContractItemInfo> contractItemInfolist) {
		this.changeContractItemInfolist = contractItemInfolist;
	}
