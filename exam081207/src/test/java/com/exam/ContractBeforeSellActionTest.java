...
import com.baoz.yx.mock.MockUser;
...
public class ContractBeforeSellActionTest extends YingXiaoBaseTest {
	...
	static private Long impSid = 1001L;
	...
	public void testSaveCBS() {
		...
		cbsAction.setImportantProjectId(impSid);
		try{
			cbsAction.saveCBS();
		}catch (Exception e){
			e.printStackTrace();
		...
		compareImpAndCbsRelation(tempC.getID());
	}
...
//		assertTrue(count>0);
	...
	private void compareImpAndCbsRelation( Long cbsId ){
		ImpAndCbsRelation  icr =  (ImpAndCbsRelation)service.uniqueResult("  from ImpAndCbsRelation icr where icr.cbsId = ? " , cbsId);
			assertEquals(impSid, icr.getImpID());
	}
}
