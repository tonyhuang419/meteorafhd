...
function Validator() {
	...
	this.map.put("dateYX",new dateYX());
	...
	this.map.put("length", new isLength());
	this.map.put("equals", new isEquals());
	this.map.put("lessToday", new lessToday());
...
function dateYX(){
	this.test = function(_value) {
		...
		if(noSpaceStr.length > 0){
			var reg = /^(\d{4})-(([1-9])|(0[1-9])|(1[0-2]))-(([1-9])|(0[1-9])|([1-2][0-9])|([3][0-1]))$/;
...
function isLength() {
	this.test = function(_value1, _value2) {
		return _value1.length  <  _value2;
	}
}

function lessToday(){
	this.test = function( _date ) {
		var today =  new Date();
		...
		var arrTodaytDate = todayStr.split("-"); 
		var arrDate = _date.split("-");  
		
		var newToday = arrTodaytDate[0] + fullZero(arrTodaytDate[1]) + fullZero(arrTodaytDate[2]);
		var newDate = arrDate[0] + fullZero(arrDate[1]) + fullZero(arrDate[2]);
		if( newDate <= newToday){
			return true;
		}
		else{
