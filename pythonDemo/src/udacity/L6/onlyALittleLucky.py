...
#        if i in listX:
#            mapX[i] = ranks[i]
#    returnList = sorted(mapX.items(), key=lambda m:m[1] ,  reverse=True )
#    return [ r[0] for r in returnList ] 
...
def ordered_search(index, ranks, keyword):
    ...
    return sorted(pages, key = lambda p: -ranks[p]) # sorts highest -> lowest, remove the - for reverse
