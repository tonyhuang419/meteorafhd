 ...
 * <br>
 * 动态加载某一文件<br>
 * XN.loadFile( 'http://xnimg.cn/js/test.js' );<br>
 *
 * 一定要在页面中写入一个如下的标签<br>
 * &lt;script vsrc="http://xnimg.cn/js/test.js?ver=$revxxx$"&gt;&lt;/script&gt;<br>
 *  这样loadFile函数将根据此标签获取正确的版本号，并在请求此文件时为其添加<br>
 *  
 *  http://xnimg.cn/156454/js/test.js<br>
 *
 * @method loadFile
 * @param {String} file
    ...
    writepipe = function( uin , nick )
    {
        ...
        {				
            ...
            window.wiw=window.open(
                    'http://xiaonei.com/webpager.do?toid=' + uin ,
                    '_blank',
...
(function()
{
    ...
    */
    ...
    var loadURI = 'http://shaft.jebe.' + domain + '/show';
    ...
    function buildAD( obj )
    {
        ...
        templete[ 2 ] = function( ad )
        {
            ...
            {
                return '<embed src="' + ad.media_uri + '" type="application/x-shockwave-flash" ' +
	'width="' + ad.width + '" height="' + ad.height + '" ></embed>';
            }
            else
            {
	            return [
    ...
    window.render_jebe_ads = function( json )
    {
        ...
        {
            if ( v.adzoneid == 1000000003 && v.ads.length !== 0 )
            {
    ...
    window.load_jebe_ads = function( s )
    {
        ...
        ready(function()
        {
            var p = getCookie( 'jebecookies' );
            if ( !p || isBlank( p ) ) p = '100|||||';
            var sc = $element( 'script' );
            sc.type = 'text/javascript';
            sc.src = loadURI + '?j=' + encodeURIComponent( p );
...
XN.DOM.readyDo(function(){
	...
	function getMessage(speak,isStar,gender){
		...
		var html=['<div style="width:370px;padding:4px;color:#EF4223;background-color:#FEFFCF;position:relative;top:-10px;left:-10px;_margin-bottom:0;">需要通过<em>',
				  user.name,
				  '</em>的验证才能加',
				  gender,
				  '为好友！</div>',
				  ...
				  '<div style="display:',
				  showUpload,
				  ';clear:both;text-align:center;"><a style="font-size:12px;" target="_blank" href="http://head.upload.xiaonei.com/Upload.do?from=friend">使用真实头像，提高请求通过率</a></div>',
...
(function()
{
    XN.app.status.notify = {
        ...
        show : function()
        {
            ...
            this.getEl( 'icon' ).show();
            this.getEl( 'icon' ).delClass( 'hide' );
        },
        ...
        update : function( str )
        {
            ...
            if ( window.statusEditor )
            {
                ...
                statusEditor.addEvent( 'updateSuccess' , function()
                {
                    this.delEvent( 'updateSuccess' , arguments.callee );
                    onSuceess();
                } );
            ...
            else
            {
                XN.loadFile( this.statusJS , function()
                {
                    ...
                    var action = new XN.app.status.updateAction({
                        reqeustURI : 'http://home.xiaonei.com/doing/update.do',
                        parseMediaURI : 'http://home.xiaonei.com/share/GetUrlInfo.do'
                    });
        ...
        sendReply : function( sid )
        {
            ...
            new XN.net.xmlhttp({
                url : 'http://home.xiaonei.com/doing/reply.do',
...
/*XN.dynamicLoad({
    file : 'http://xnimg.cn/jspro/xn.app.share.js',
    funcs : [ 'pop_share' ]
});
*/
