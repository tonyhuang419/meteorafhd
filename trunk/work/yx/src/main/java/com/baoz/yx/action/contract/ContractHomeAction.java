...
public class ContractHomeAction extends DispatchAction {
	...
	private List<ContractMainInfo>contractNames=new ArrayList<ContractMainInfo>();
	private List<YXClientCode>clients=new ArrayList<YXClientCode>();
	...
	public String goLeftTopJsp()throws Exception{
		...
		sp.append("select c,y from ContractMainInfo c,YXClientCode y where c.saleMan="+empId+" and c.conCustomer=y.id");
		sp.append(" and (c.conState=0 or c.conState=1 or c.conState=2)");
		//String hqlSp="select y from YXClientCode y";
		//clients=(List<YXClientCode>)commonService.list(hqlSp);
		//List<YXClientCode> clientts=(List<YXClientCode>)systemService.queryClients(empId);
		
		List<Object> list=(List<Object>)commonService.list(sp.toString());
		for(int i=0;i<list.size();i++){
			Object[]ara=(Object[])list.get(i);
			YXClientCode y=(YXClientCode) ara[1];
			ContractMainInfo c=(ContractMainInfo) ara[0];
			boolean hasClient = false;
			for (YXClientCode client : clients) {
				if(client.getId().equals(y.getId())){
					hasClient = true;
				}
			}
			if(!hasClient){
