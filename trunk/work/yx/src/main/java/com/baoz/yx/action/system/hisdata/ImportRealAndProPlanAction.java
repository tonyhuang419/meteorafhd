...
import com.baoz.yx.tools.Combination;
import com.baoz.yx.vo.ExcelContractGatheringHistory;
import com.opensymphony.xwork2.ActionSupport;

@Results({
	@Result(name="success", value="/WEB-INF/jsp/system/hisdata/importHisDataRealAndPro.jsp")
})
public class ImportRealAndProPlanAction extends ActionSupport {
		...
		if (excelFile != null) {
			...
			for (int i = 0; i < sheet.getRow(28).length; i++) {
				...
				if (StringUtils.equals(cellName.getContents(), "发票号")) {
					rowRule.addCellRule(new CellRuleImpl(i, "invoiceCode",
							new StringArrayCellValueConvertor(true)));
					continue;
				}
				...
				if(StringUtils.equals(cellName.getContents(), "实际开票金额")){
					rowRule.addCellRule(new CellRuleImpl(i,"realBillAmount",
							new NumberCellValueConvertor()));
					continue;
				}
			...
			for (RowResult<ExcelContractGatheringHistory> rowResult : cons) {
				if (!rowResult.hasErrors()) {
					...
					if(StringUtils.isBlank(excelContract.getInvoiceType())){
						logger.error(getRowMessage(rowResult) + ",合同号为"
								+ excelContract.getContractCode()
								+ ",的发票类型为空");
						continue;
					}
					if(excelContract.getInvoiceType().equals("普票")){
						excelContract.setInvoiceType("服务业普通发票");
					}else if(excelContract.getInvoiceType().equals("增票")){
						excelContract.setInvoiceType("增值税发票");
					}
					String invoiceType=typeManageService.getYXTypeSmallByName(1004L,excelContract.getInvoiceType());
					if(invoiceType==null){
						logger.error(getRowMessage(rowResult) + ",合同号为"
							+ excelContract.getContractCode()
							+ ",的发票类型："
							+ excelContract.getInvoiceType()
							+ "不存在");
						continue;
					}
					excelContract.setInvoiceType(invoiceType);
					...
					List<RealContractBillandRecePlan> initProList=new ArrayList<RealContractBillandRecePlan>();
					...
					if (contract.getContractType().equals("1")) {
						...
						if(excelContract.getRealBillAmount()==null){
							excelContract.setRealBillAmount(0.0);
						}
						
						String planHql = "select plan from RealContractBillandRecePlan plan"+
										" where plan.conMainInfoSid =?"+ 
										" and plan.contractItemMaininfo =? and plan.conItemStage =?" +
										" and plan.billType =?";
							 initProList = service
									.list(planHql, contract.getConMainInfoSid(), itemMainInfo.getConItemMinfoSid(), itemStage.getConIStageSid(),invoiceType);
					} else{
						String itemStageName = typeManageService
						.getYXTypeSmallByName(1023L, excelContract
								.getGatheringPhase());
						...
						String planHql = "select plan from RealContractBillandRecePlan plan"
							+ " where plan.conMainInfoSid =? "
							+ "  and plan.conItemStage =? and plan.billType=?";
						initProList = service
							.list(planHql, contract.getConMainInfoSid(), itemStage.getConIStageSid(),invoiceType);
					}
					
					double invoiceMoney = excelContract.getRealBillAmount()== null ? 0.0 : excelContract.getRealBillAmount();//实际开票金额
					ExcelContractGatheringHistory history=new ExcelContractGatheringHistory();
					BeanUtils.copyProperties(history, excelContract);
				
						List lastList=new ArrayList();
						boolean flag=false;
						for(int k=1;k<=initProList.size();k++){
							List list=(Combination.combination(initProList, k));
							for (Object object : list) {
								List opList=(List)object;
								double sum=0.0;
								for (Object object2 : opList) {
									sum+=((RealContractBillandRecePlan)object2).getRealBillAmount().doubleValue();
								}
								if(sum==invoiceMoney){
									for (Object object2 : opList) {
										lastList.add(object2);
									}
									flag=true;
									break;
								}
						...
						if(lastList!=null&&lastList.size()>0){
							this.importHisDataService.importBillAndRecePlan(history, lastList, contract);
						}else{
							logger.error(getRowMessage(rowResult) + ",合同号为"
								+ excelContract.getContractCode()
								+ ",阶段名称为："
								+ excelContract.getGatheringPhase()
								+ "，发票类型为："+excelContract.getInvoiceType()+",的发票开票金额没有符合的实际开票收款计划");
						}
				} else {
