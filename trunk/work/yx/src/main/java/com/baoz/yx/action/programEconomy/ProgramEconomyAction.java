...
public class ProgramEconomyAction extends DispatchAction {
	...
	private ContractBeforeSell cmain;
	...
	public String projectInfo() {
		...
		this.proe = o;
		if(o.getShellContractId()!=null||o.getShellContractId()!=0)
		    cmain=(ContractBeforeSell) commonService.load(ContractBeforeSell.class, o.getShellContractId());
		return "pinfo";
	}
	...
	public String saveBid() {
		...
		{
		 service.save(bid);
		 YXOProgramEconomy x=(YXOProgramEconomy)service.load(YXOProgramEconomy.class, bid.getProjectEconomyId());
		 x.setBidVerify("1");
		 x.setEnterTime(new Date());//时间
		 service.update(x);
		}
		if (eid == null || !"".equals(eid)) {
			this.commonLists(eid);
	...
	public ContractBeforeSell getCmain() {
		return cmain;
	}

	public void setCmain(ContractBeforeSell cmain) {
		this.cmain = cmain;
	}
}
