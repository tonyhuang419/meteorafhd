...
public class ClientLinkManAction extends DispatchAction implements ServletRequestAware {
	...
	private Long nameid;	
	private String from;  //默认：联系人管理     1:客户管理 
	...
	public String updateExp() throws Exception {
		...
		if (cc.getId() != null && !"".equals(cc.getId())) {
			YXLinkMan linkMain = (YXLinkMan) service.load(YXLinkMan.class, cc.getId());
			Date d = new Date();
			linkMain.setUpdateBy(d);
			linkMain.setById(UserUtils.getUser().getId());
			/////
			linkMain.setAddress(cc.getAddress());
			linkMain.setCallPhone(cc.getCallPhone());
			linkMain.setClientId(cc.getClientId());
			linkMain.setEmail(cc.getEmail());
			linkMain.setName(cc.getName());
			linkMain.setNatureId(cc.getNatureId());
			linkMain.setPhone(cc.getPhone());
			linkMain.setOtherPhone(cc.getOtherPhone());
			/////
			service.update(linkMain);
			return "success";
		} else {
