...
@Results( {
		...
		@Result(name = "updateSuccess", value = "/commons/jsp/reloadOpenerAndClose.jsp"), 
		@Result(name = "distributeAuthoritySuccess", value = "/commons/jsp/closeAndCallBackOpener.jsp"), 
...
//	private List roles;
	...
	public String saveRole() throws Exception {
        ...
        sp.append("from Role where ( code='"+role.getCode()+"' or roleName='"+role.getRoleName()+"' ) ");
        if(role.getId() != null){
        	sp.append(" and id <> "+role.getId());
        }
        List<Role>rolelist=(List<Role>)commonService.list(sp.toString());
        if(rolelist.size()<1)
        {
            roleService.saveOrUpdatRole(role);
            return "updateSuccess";
        }else{
        	ActionContext.getContext().put("codeOrNameDuplicate", true);
        	return ENTER_SAVE;
        }
	}
	...
	public String saveRoleAuthority() throws Exception {
		...
		return "distributeAuthoritySuccess";
	}
