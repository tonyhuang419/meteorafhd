 ...
 */
@Results( {	
	@Result(name = "success", value = "/WEB-INF/jsp/statistics/billAccuracyStatisticsList.jsp"),
	@Result(name = "showQuery", value = "/WEB-INF/jsp/statistics/billAccuracyStatisticsQuery.jsp"),
	@Result(name = "showBillDetial", value = "/WEB-INF/jsp/statistics/billAccuracyStatisticsDetial.jsp")
})
public class BillAccuracyStatisticsQueryAction  extends DispatchAction {
	@Autowired
	...
	private ObjectPropertySessionHoldTool holdTool = new ObjectPropertySessionHoldTool(
    		"billAccuracyStatisticsQueryParameters",this,new String[]{"groupId","saleMan","yearSel","monthSel"}); 
	@Override
	...
	public String queryResult(){
		 ...
		 *  计算计划内预计笔数   1
		 */
		select.append(" (select count(*) from MonthlyBillpro mb,RealContractBillandRecePlan t,ContractMainInfo cm where " +
		...
		select.append(" ),");
		// 计算计划额度    2
		select.append(" (select sum(mb.billTaxAmount) from MonthlyBillpro mb,RealContractBillandRecePlan t,ContractMainInfo cm " +
		 ...
		 * 实际开票笔数   3
		 */
		select.append(" (select count(*) from MonthlyBillpro mb,RealContractBillandRecePlan t,ContractMainInfo cm " +
				"where t.conMainInfoSid = cm.conMainInfoSid and mb.planBillAmount > 0 " +
				"and mb.realContractBillandRecePlan = t.realConBillproSid " +
				"and cm.saleMan = emp.id " +
				"");
		//按照实际卡票时间来查询
		...
		select.append(" ),");
		//实际开票额度      4
		select.append(" (select sum(mb.planBillAmount) from MonthlyBillpro mb,RealContractBillandRecePlan t,ContractMainInfo cm " +
				"where t.conMainInfoSid = cm.conMainInfoSid and mb.planBillAmount > 0 " +
				"and mb.realContractBillandRecePlan = t.realConBillproSid " +
				"and cm.saleMan = emp.id " +
				"");
		...
		sql.append(" and (select count(*) from MonthlyBillpro mb,RealContractBillandRecePlan t,ContractMainInfo cm " +
				"where t.conMainInfoSid = cm.conMainInfoSid " +
				"and cm.saleMan = emp.id " +
				"and mb.realContractBillandRecePlan = t.realConBillproSid ");
		//日期范内围
		...
		if(exportX!=null && exportX.equals("1")){
			List<Object[]> objList  = commonService.list(select.append(sql).toString() );
		...
		countPlan = applyBillService.totalPlaninside(groupId, saleMan, yearSel, monthSel);
		totalReal = applyBillService.totalReal(groupId, saleMan, yearSel, monthSel);
		totalRealPlan = applyBillService.totalRealInside(groupId, saleMan, yearSel, monthSel);
		return "success";
	}
	...
	private  void processExport(List<Object[]> oArray) {	
		...
		export.addTitle(new String[]{"销售员","计划内开票笔数","计划内额度","实际计划内开票笔数","实际计划内额度","实际开票笔数","实际额度"
				,"计划外开票笔数","计划外额度","计划内笔数%","计划内额度%","计划外笔数/计划内笔数%","计划外额度/计划内额度%"});
		export.setTableName("开票精度统计");
		...
		OutputStream os = DownloadUtils.getResponseOutput("开票精度统计导出.xls");
		export.export(os);
	 ...
	 * @return
	 */
	public String showBillDetial(){
		info = applyBillService.billDetialQuery(info, saleMan, yearSel, monthSel);
		return "showBillDetial";
	}
