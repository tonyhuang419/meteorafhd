...
@Results({
	@Result(name="success",value="/WEB-INF/jsp/system/hisdata/updateBillAndReceDate.jsp")
	
})
public class UpdateBillAndReceDateAction extends ActionSupport{
	private static final long serialVersionUID = 1L;

	private static Logger logger = Logger
			.getLogger(UpdateBillAndReceDateAction.class);
...
//			ROWRULE.ADDCELLRULE(NEW CELLRULEIMPL("I", "BILLTYPE"));
			...
			for (RowResult<ExcelCompareRealPlanHistory> rowResult : cons) {
				if (!rowResult.hasErrors()) {
					...
					if(history.getRealPlanId().equals(1251)){
						logger.error("nihao");
					}
					...
					importHisDataService.updateRealBillAndReceDate(
							plan,history.getRealBillDate(),history.getRealReceiptDate());
				} else {
