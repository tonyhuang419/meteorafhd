...
function HS_calender() {
	...
	if (arguments.length > 1) {
		...
		var CalenderBox = style
				...
				+ lis
				+ "</ul></div><div class='calenderBottom'><a href='javascript:void(0)' class='closeCalender' onclick='closeCalender(this)'>&times;</a><span><span><a href=javascript:void(0) onclick='_selectThisDay(this)' title='"
...
function _selectThisDay(d) {
	...
	var dat = d.title;
	var y = dat.substring(0, dat.indexOf("-"));
	var m = dat.substring(dat.indexOf("-") + 1, dat.lastIndexOf("-"));
	var d = dat.substring(dat.lastIndexOf("-") + 1, dat.length);
	dat = y;
	if (m.length == 1) {
		dat = dat + "-0" + m;
	} else {
