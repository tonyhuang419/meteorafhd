...
function getEmployeeOfDepartment(departmentCode){
		ajaxSetSelectOptions("/yx/jsonData.action?method=doGetEmployeeOfDepartment2&departmentCode="+departmentCode,document.noContractReve.expId,"id","name",{value:"",text:"    "});
}
...
<table width="100%"  class="bg_table02" >
	      ...
	      <tr class="bg_table02">
			      <td align="right" nowrap>收款状态：</td>
		          <td class="bg_table02" align="left" >
		          	<select name="reveState">
						<option value="">全部</option>
						<option value="0" <s:if test="reveState == 0">selected</s:if> >未核销</option>
						<option value="1" <s:if test="reveState == 1">selected</s:if> >已核销</option>
						<option value="2" <s:if test="reveState == 2">selected</s:if> >历史核销</option>
					</select>
				</td>
	      </tr>
