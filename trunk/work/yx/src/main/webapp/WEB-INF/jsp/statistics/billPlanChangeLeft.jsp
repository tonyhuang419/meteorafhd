...
<script type="text/javascript">
	function getEmployeeOfDepartment(departmentCode){
		ajaxSetSelectOptions("<s:url includeParams="none" action="jsonData"></s:url>?method=doGetEmployeeOfDepartmentSimple&departmentCode="+departmentCode,document.billProjectChangeStat.saleMan,"id","name",{value:"",text:"    "});
	}
	function selectedClient(clientObj){
		document.billProjectChangeStat.customerName.value = clientObj.clientFullName;
		document.billProjectChangeStat.customer.value = clientObj.clientId;
	}
	function cleanValue(obj){
		if(obj==null||obj.length==0){
			document.billProjectChangeStat.customer.value="";
		}
	...
	function onloadMethod(sale)
	{
		...
		var departmentCode=document.billProjectChangeStat.groupId.value;
		ajaxSetSelectOptions("<s:url includeParams="none" action="jsonData"></s:url>?method=doGetEmployeeOfDepartmentSimple&departmentCode="+departmentCode,document.billProjectChangeStat.saleMan,"id","name",{defaultAsync:false,value:"",text:"    "});
...
<s:form action="billProjectChangeStat" theme="simple" target="main_view">
