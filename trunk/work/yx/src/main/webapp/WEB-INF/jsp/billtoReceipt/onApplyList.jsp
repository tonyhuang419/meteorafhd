			...
			<table align="center" border=1 cellpadding="1" cellspacing=1
				...
				<input type="hidden" name="realPanid" value="<s:property value="#month[0].realConBillproSid"/>"/>
					<tr align="center" class="bg_table03">
						...
						<td align="right">
						<s:property value="#month[0].realBillAmount" /></td><s:hidden name="realBillAmount" value="%{#month[0].realTaxBillAmount}" />
					</tr>
					<tr class="bg_table02">
					<tD></tD>
					<td></td>
					...
					<td colspan="3">
						<s:textarea id="content%{#status.index}" name="billContent" value="%{#month[0].billContent}" rows="3" cols="30"/></td>
					<td>备注:</td>
					<td colspan="2">
						<s:textarea name="applyBillList[%{#planIndex}].remarks" value="%{#month[2].partyAConId}/%{#month[5]}" rows="3" cols="30"/></td>
					</td>
					</tr>
					<tr class="bg_table02">
					<td colspan="2"></td>
					<td>库存组织:</td>
					<td colspan="2">
					<s:select name="stockOrgList" list="stockOrgList" listKey="typeSmall" listValue="typeName" required="true"></s:select>
					<td>是否一次出库:</td>
					<td>
						<s:checkbox name="applyBillList[%{#planIndex}].oneOut"></s:checkbox>
					</td>
					<td>
						取票地点:
					</td>
					<td>
						<s:textfield name="applyBillList[%{#planIndex}].billSpot" value="宝山"></s:textfield>
					</td>
					</tr>
					<tr class="bg_table02">
					<td colspan="2"></td>
					<td>甲方接收人:</td>
					<td colspan="2">
						<s:textarea name="applyBillList[%{#planIndex}].firstReceiveMan" rows="3" cols="30"/></td>
					</td>
					<td colspan="4"></td>
					</tr>
