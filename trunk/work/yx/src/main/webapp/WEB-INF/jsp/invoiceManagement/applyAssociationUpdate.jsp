...
<script language="javascript">
	...
	//通过合同号有无，判断是修改已签、未签申请
		...
		if(consysid.length < 1 ){
			document.forms(0).action="createInvoice.action?method=applicationsAssociationDelete";
		}
		else{
			document.forms(0).action="/yx/contract/formalContractManage/formalContractInvoiceApply.action?modify=true&method=applicationsAssociation";
		}
	...
	//通过合同号有无，判断是修改已签、未签申请
		var consysid = document.getElementById("consid").value;
		if(consysid.length < 1){
			document.forms(0).action="createInvoice.action?method=findAssociationUpdate&iid="+document.getElementById("danhao").value;
		}
		else{
