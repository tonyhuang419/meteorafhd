...
<body  onload="refresh()"><div align="left">
 <div  style="color:#000000">当前页面：合同管理 -&gt; 正式合同管理 -&gt;合同变更</div>
</div>
...
<table width="100%" height="100%" border="0"  align="center" cellpadding="1" cellspacing="1" class="bg_table02">
  ...
  <tr class="bg_table02">
    <td colspan="4" align="center" class="bg_table02"><div id="container" class="bg_table02">
            ...
            <div align="center">
              <table  width="100%" height="307">
                ...
                <tr>
                  ...
                  <td class="bg_table02" align="left"><div align="left">
                 <s:select list="#@java.util.TreeMap@{0:'非预决算',1:'预决算'}"   name="maininfo.FinalAccount"  disabled="true"></s:select> </div></td>
             ...
             <td colspan="4" align="right" class="bg_table02"><hr align="right"></td>
                ...
                <s:iterator value="mainMoneyList" status="mainMoneyList" >
                  ...
                  <td align="center"><s:select list="tickettype" name="ticketType" listKey="typeSmall" listValue="typeName" ></s:select>
                  <a href="javascript:if(!validate1()){setNum(1);doSubmit();}">添加</a>
                  </td>
                  </tr>
        ...
        <div align="center">
        <input type="button" value="保存" onclick="if(!validate()){setNum(1);document.forms(0).submit();}" class="button01">
 		<input  type="button" value="申请提交"  onClick="if(!validate()){setNum(1);tijiao();}" class="button01" > 
...
function validate1()
{
	...
	var opSum=0;
	ev2.test("float","合同费用总金额必须为大于0的数字！",document.formalContractModify.maininfoMoney.value);
	ev2.writeErrors(errorsFrame, "errorsFrame");
	if(ev2.size()>0){
			return true;
	}
	var totalMoney=parseFloatNumber(document.formalContractModify.elements("maininfoMoney").value);
	if(k!=null&&k>0){
		for(var i=0;i<k;i++){
		ev2.test("float","费用组成金额必须为大于0的数字",document.formalContractModify.elements("mainMoneyList["+i+"].money").value);
		ev2.writeErrors(errorsFrame, "errorsFrame");
		if(ev2.size()>0){
			return true;
		}
		var num=parseFloatNumber(document.formalContractModify.elements("mainMoneyList["+i+"].money").value);
		opSum+=num;
		}
	}
	if(opSum>totalMoney)
...
function tijiao()
{
	document.formalContractModify.method.value="applySubmitByMainInfo";
	document.formalContractModify.submit();
}
function validate(){
     var ev2=new Validator();
     //验证金额格式是否正确
     var totalMoney = document.formalContractModify.maininfoMoney.value;
     ev2.test("float","合同总金额必须为大于0的数字！",totalMoney);
     var count= document.formalContractModify.mainInfoPartIndex.value;
     for(var k=0;k<count;k++){
     	var partMoney = document.formalContractModify.elements("mainMoneyList["+k+"].money").value;
...
function doSubmit()
{
	var ev2=new Validator();
	 ...
	 var tempType=document.formalContractModify.mainmoneytype.value;
	 ev2.test("notblank","费用组成没有选择",tempType);
	 <s:iterator value="mainMoneyList" status="mainMoneyList" >
	...
	ev2.test("float","合同费用总金额必须为大于0的数字！",$('mainmoney').value);
	 ev2.writeErrors(errorsFrame, "errorsFrame");
	 if(ev2.size()>0)
	 {
	 	return;
	 }
	 document.formalContractModify.method.value = "addMainInfoPart";
	document.formalContractModify.submit();
}
