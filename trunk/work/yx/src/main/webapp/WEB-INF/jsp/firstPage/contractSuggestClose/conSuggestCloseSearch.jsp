...
<html>
<head>
<title>合同建议关闭搜索</title>

<script language="javascript">
	function getEmployeeOfDepartment(departmentCode){
		ajaxSetSelectOptions("<s:url action="jsonData"></s:url>?method=doGetEmployeeOfDepartment&departmentCode="+departmentCode,document.contractSuggestClose.expId,"id","name",{value:"",text:"    "});
	}
	...
	function getClientOfEmployee(employeeId){
		//ajaxSetSelectOptions("<s:url action="jsonData"></s:url>?method=doGetClientOfEmployee&employeeId="+employeeId,document.contractSuggestClose.clientSelectId,"id","name",{value:"",text:"    "});
	}
	function clean()
	{
