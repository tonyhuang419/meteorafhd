...
<!--								<s:property value="srr.sectionName" />-->
											...
											<s:iterator value="recordInfo" status="status">
												...
												<s:else>
													<tr class="bg_table02">
														...
														<td class="bg_table02">&nbsp;</td>
														<td class="bg_table02">
														<div align="center"><s:property value="typeManageService.getYXTypeManage(1012,typeCodee).typeName" /></div>
														</td>
														<td class="bg_table02">
					...
					<s:hidden name="sr.bpmsFlag"></s:hidden> <s:if test="bpmsF==2">
						<s:if test="srr.bpmsFlag==0">
						      <s:if test="infoList.size()>0">
							   <input type="submit" value="BPMS录入确认"
						...
						<s:if test="srr.bpmsFlag==1">
						        <s:if test="infoList.size()>0">
							       <input type="submit" value="BPMS录入取消"
							...
							<table width="100%">
									...
									<tr>
										...
										<s:if test="equipList.size() == #status.index + 1">
											...
											<div align="center"><a
												...
												onclick="javascript:if(confirm('确定要删除吗？')){return true} else{return false}">删除</a>
								...
								<tr>
									...
									<div align="center"><input type="text"
										id="equipenterTime" name="equip.enterTime" 
										 size="12" /> <img
					...
					<table width="100%" id="tab1">
						...
						<tr>
							...
							<td align="right" class="bg_table02">
							<div align="center"><font color="red">* </font><input type="text" size="12" id="bidName"
								name="bid.bidName" /></div>
							</td>

							<td align="left" nowrap class="bg_table02">
							<div align="center">
							<div align="center"><font color="red">* </font><input type="text" name="bid.desc" id="bidDesc"
								size="12"></div>
							</div>
							</td>
							<td align="left" nowrap class="bg_table02"><font color="red">* </font><input
...
function validate2()
	{
        ...
        with(programEconomy){
            ...
            ev.test("dateYX","录入时间格式不正确!",$("equipenterTime").value);
		   }
		if (ev.size() > 0) {
			ev.writeErrors(errorsFrame, "errorsFrame");
			return true;
...
function validate3()
	{
		var ev = new Validator();
        with(programEconomy){
            ev.test("notblank", "招标文件不能为空", $("bid.bidName").value);
            ev.test("notblank", "描述不能为空", $("bid.desc").value);
            ev.test("notblank", "录入时间不能为空",$("bid.enterTime").value);
            ev.test("dateYX","录入时间格式不正确!",$("bid.enterTime").value);
