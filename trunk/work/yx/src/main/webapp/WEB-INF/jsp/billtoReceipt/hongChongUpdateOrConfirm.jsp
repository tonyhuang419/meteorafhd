...
<script type="text/javascript">
	function saveSubmit(cal,planLength){
		if(cal == 1){
			var flag ;
			if(planLength < 1){
				flag = true;
			}else{
				flag = checkSumInput(planLength);
			}
			if(flag){
				...
				alert("你修改后的值与开票金额不符合不能提交申请")
			}
		}else{
			document.hongChongQuery.method.value="hongChongBillService";
			document.hongChongQuery.submit();
			window.close();
		}
	}
...
<s:form action="hongChongQuery" theme="simple" target="rightFrame">
	<s:hidden name="method" value="submitApply"></s:hidden>
	<table align="center" width="100%">
		<tr>
			<td>
				<div align="left" style="color:#000000">首页：退票>退票修改确认</div>
			</td>
		</tr>	
		<tr>
			<td>
		<s:iterator value="invoiceBillList" id="bill">
			...
			<table width="100%" border="1" class="bg_table02" align="center" bordercolor="#808080" style=" border-collapse: collapse;">
				...
				<tr class="">
					...
					<s:property value="#bill[1].invoiceAmount" /></td>
		...
		<s:if test="hongChongPlanList.size() != 0 " >
			...
			<table width="100%" border="1" class="bg_table02" align="center" bordercolor="#808080" style=" border-collapse: collapse;">
				...
				<s:iterator value="hongChongPlanList" id="month" status="status">
					<tr align="center" onMouseOver="this.bgColor='#BBBBFF'; "	onMouseOut="this.bgColor='#FFFFFF';">
						...
						<td align="right">
							...
							<s:if test="#month[5].realContractBillandRecePlan == #month[0].realConBillproSid">
								<s:if test="#parameters.isConfirm[0] == 1">
									<s:textfield id="hongChongRelation%{#status.index}" name="hongChongRelation[%{#status.index}].relateAmount" value="%{#month[5].relateAmount}" size="10"
									onblur="formatInputNumber(this);checkInput(this,'%{hongChongPlanList.size}','%{#month[0].billInvoiceAmount}')"></s:textfield>
								</s:if>
								<s:else>
									<s:textfield disabled="true" name="hongChongRelation[%{#status.index}].relateAmount" value="%{#month[5].relateAmount}" size="10"></s:textfield>
								</s:else>
							</s:if>
						</td>
				...
				<tr class="bg_table01">
					...
					<s:if test="#parameters.isConfirm[0] == 1 ">
						<input type="button" name="1" value="确认申请" class="button01" onclick="saveSubmit(1,'<s:property value="hongChongPlanList.size()"/>')">
					</s:if>
					<s:else>
						<input type="button" name="3" value="确认退票" class="button01" onclick="saveSubmit(2)">
					</s:else>
