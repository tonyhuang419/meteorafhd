...
{
  public AvResult getAvailByFltNo(String fltNo, String firstSegDate)
    throws Exception
  {
    return 
      getAvailability(fltNo.toUpperCase(), 
      QDateTime.stringToDate(firstSegDate, "YYYYMMDD HH:MI:SS"));
  }
    ...
    throws Exception
  {
    try
    {
      return getAvailability(airline.toUpperCase(), 
    ...
    throws Exception
  {
    try
    {
      return getAvailability(org.toUpperCase(), dst.toUpperCase(), 
        QDateTime.stringToDate(date, "YYYYMMDD HH:MI:SS"), "ALL", 
        false, false);
    } catch (Exception e) {
      throw e;
    }
    ...
    throws Exception
  {
    try
    {
      return getAvailability(org.toUpperCase(), dst.toUpperCase(), 
        QDateTime.stringToDate(date, "YYYYMMDD HH:MI:SS"), 
        airline.toUpperCase(), false, false);
    } catch (Exception e) {
      throw e;
    }
  }
    ...
    throws Exception
  {
    try
    {
      return getAvailability(org.toUpperCase(), dst.toUpperCase(), 
        QDateTime.stringToDate(date, "YYYYMMDD HH:MI:SS"), 
        airline.toUpperCase(), direct, false);
    } catch (Exception e) {
      throw e;
    }
  }
    ...
    throws Exception
  {
    try
    {
      return getAvailability(org.toUpperCase(), dst.toUpperCase(), date, 
        "ALL", false, false);
    } catch (Exception e) {
      throw e;
    }
    ...
    throws Exception
  {
    try
    {
      return getAvailability(org.toUpperCase(), dst.toUpperCase(), 
        QDateTime.stringToDate(date, "YYYYMMDD HH:MI:SS"), 
        airline.toUpperCase(), direct, e_ticket);
    } catch (Exception e) {
      throw e;
    }
  }
  ...
  public AvResult getAvailability(String org, String dst, Date date, String airline, boolean direct, boolean e_ticket, boolean nonstop)
    throws Exception
  {
    AvResult rr = getAvailability(org, dst, date, airline, direct, e_ticket);
    if (nonstop)
      rr.ApplyFilter(new AVFilter(this)
      {
        public boolean test() {
    ...
    try {
      ...
      for (int i = 0; i < items.length; ++i) {
        ...
        for (int j = 0; j < segments.length; ++j) {
          ...
          case '+':
            as.arridate = 
              new Date(ar.date.getTime() + 
              (segment[2].charAt(1) - '0') * 86400000);
            break;
          case '-':
            as.arridate = 
              new Date(ar.date.getTime() - 
          ...
          case ',':
          default:
            as.arridate = ar.date; break label453:

            as.arridate = ar.date; }
          label453: String arrStr = QDateTime.dateToString(as.arridate, 
            "yyyymmdd");
          arrStr = arrStr + ((as.arritime == null) ? "0000" : as.arritime);
          as.arridate = 
            QDateTime.stringToDate(arrStr, "yyyymmddhhmi");
          ...
          case ',':
          default:
            as.depdate = ar.date; break label681:

            as.depdate = ar.date;
          }
          label681: String depStr = QDateTime.dateToString(as.depdate, 
            "yyyymmdd");
          ...
          for (char c = 'A'; c <= 'Z'; c = (char)(c + '\1'))
            if (segment[15].charAt(c - 'A') != '-')
              as.putCangwei(c, String.valueOf(
                segment[15].charAt(c - 'A')));


          as.setCangweiNumber(26);
          for (int k = 0; k < 26; ++k)
            as.putCangweiSort(segment[16].charAt(k), 
              segment[17].charAt(k));
