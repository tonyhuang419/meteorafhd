...
public class ShowSelectRelationAction extends DispatchAction implements ServletRequestAware{
	...
	public String doDefault() throws Exception {
		...
		info=queryService.listQueryResult(" from InvoiceInfo ii where ii.type != 4 and ii.byId = '"+uid+"'",info);
		logger.info(ids);
		invoiceList=service.list(" from InvoiceInfo ii where ii.id = "+ids);
		logger.info(invoiceList.get(0).getInvoiceAmount());
		
		double sum = invoiceList.get(0).getInvoiceAmount();
		
		relationList=service.list(" from RelationAmount ra where ra.invoiceToReceip = " + ids);
