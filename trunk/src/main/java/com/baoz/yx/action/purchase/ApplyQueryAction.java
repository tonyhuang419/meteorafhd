...
public class ApplyQueryAction extends DispatchAction {
	@Autowired
  ...
  //查询条件  销售员,审购类型（2种）,客户,审购日期
	...
	public String doDefault() throws Exception {
		...
		str.append("from ApplyMessage am where  am.id=1 ");
		...
		if(affType!=null&&!"".equals(affType)){
			str.append("and am.affirmState= ").append(affType);
		}
		if(name!=null&&!"".equals(name)){
			str.append("and am.applyContent like '%").append(name).append("%'");
		}
