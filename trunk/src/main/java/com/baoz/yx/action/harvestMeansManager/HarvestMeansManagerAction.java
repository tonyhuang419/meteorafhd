...
public class HarvestMeansManagerAction extends DispatchAction implements
	...
	public String saveAmount() throws Exception {
		...
		Double amouts =(Double) service.uniqueResult("select sum(ri.amount) from ReveInfo ri where ri.billSid = '" + ids + "' and ri.is_active =1");
...
//        harvestservice.delreve(Long.valueOf(del));
	...
	public String updateAmount() throws Exception {
		...
		if (ri.getId() != null && !"".equals(ri .getId())) {
			ReveInfo toUpdate = (ReveInfo) service.load(ReveInfo.class, ri.getId());
			Long uid = UserUtils.getUser().getId();
			if (uid != null && !"".equals(uid))
	...
	public String del() {
        ...
        harvestservice.delreve(Long.valueOf(ids));
        Double amouts =(Double) service.uniqueResult("select sum(ri.amount) from ReveInfo ri where ri.billSid = '" + id + "' and ri.is_active =1");
