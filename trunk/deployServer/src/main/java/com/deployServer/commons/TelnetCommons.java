...
public class TelnetCommons {
	...
	public static void unzipUploadFiles (){
		try {
			AutomatedTelnetClient telnet = new AutomatedTelnetClient( ConstantUtil.getIp() , "jboss", ConstantUtil.getPassword() );
...
//			System.out.println(telnet.sendCommand("ps -ef"));
			...
			logger.info(telnet.sendCommand(ConstantUtil.getBaseUrl()));
